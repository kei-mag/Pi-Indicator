...
def init(pi, i2c_handler):
    ...
    #   - C: カーソル表示 (0)
    #   - B: ブリンク表示 (0)
    write_command(pi, i2c_handler, 0b00001111)
...
def main(pi, i2c_handler):
    ...
    # IS=0, RE=0, SD=0
    write_command(pi, i2c_handler, 0b00101010)  # IS=0, RE=1, SD=0
    write_command(pi, i2c_handler, 0b00000101)  # 末尾2ビット 通常 10 / ディスプレイ上下反転 01
    write_command(pi, i2c_handler, 0b00101000)  # IS=0, RE=0, SD=0

    ddram_addr = 0x44  # 1行目の先頭 (0x00) 逆ディスプレイは0x44
    write_command(pi, i2c_handler, 0b10000000 | ddram_addr)  # Set DDRAM RAM Address
    l1 = b"1234567890123456"
    for char in l1:
        sleep(0.25)
    ...
    ddram_addr = 0x64  # 2行目の先頭 (0x20) 逆ディスプレイは0x64
    write_command(pi, i2c_handler, 0b10000000 | ddram_addr)  # Set DDRAM RAM Address
    l2 = b"abcdefghijklmnop"
    for ch in l2:
        sleep(0.25)
        write_data(pi, i2c_handler, ch)
        

def main2(pi, i2c_handler):
    """
    繰り返し表示
    """
    # 設定
...
def main3(pi, i2c_handler):
    ...
    write_command(pi, i2c_handler, 0b00101010)  # IS=0, RE=1, SD=0
    # write_command(pi, i2c_handler, 0b00000101)  # 逆ディスプレイ
    ...
    try:
        l2 = b"If there was an error the number of bytes read will be less than zero (and will contain the error )."
        l2 += (b" " * (len(l2) % 20))
        idx = 0
        while (True):
            for char in l2:
...
if __name__ == "__main__":
    ...
    try:
        init(pi, i2c_handler)
        main(pi, i2c_handler)
        # main2(pi, i2c_handler)
        # main3(pi, i2c_handler)
        sleep(10)
    finally:
        off(pi, i2c_handler)
        sleep(1)
